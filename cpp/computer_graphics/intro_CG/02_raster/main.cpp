...
void test_polygon_intersection() {
    ...
    Polygon pg1 {{100, 80}, {120, 40}, {200, 10}, {350, 75}, {310, 85}};
    pg1.push_back({255, 40}); pg1.push_back({225, 90});
    pg1.push_back({185, 90}); pg1.push_back({205, 45});
    pg1.push_back({215, 85}); pg1.push_back({235, 35});
    pg1.push_back({165, 30}); pg1.push_back({150, 80});
    pg1.push_back({135, 95}); pg1.push_back({115, 100});

    std::reverse(std::begin(pg1), std::end(pg1));

    //fill_poly_scan(I, pg1, PPM_Color{60, 90, 255});
    pg1.draw(I, PPM_Color{216, 174, 52});

    Polygon pg2 {{140, 10}, {270, 20}, {280, 110}, {170, 115}, {100, 95}};
    std::reverse(std::begin(pg2), std::end(pg2));
    pg2.draw(I, PPM_Color{2, 224, 2});

    I.write_to("polygons.ppm");
}
