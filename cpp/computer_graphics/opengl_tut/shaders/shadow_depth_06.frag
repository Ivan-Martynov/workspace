...
float calculate_shadow(vec4 light_frag_pos, float shadow_bias) {
    ...
    for (int x = -1; x <= 1; ++x)
        for (int y = -1; y <= 1; ++y) {
            float pcf_depth = texture(shadow_map, proj_coords.xy + vec2(x, y) *
                    texel_size).r;
            shadow_val += (current_depth - shadow_bias > pcf_depth) ? 1.0f : 0.0f;
        }
    shadow_val /= 9.0f;
...
void main() {
    ...
    // shadow part
    float bias_val = max(0.05 * (1 - dot(normal_vec, light_dir)), 0.005);
    float shadow_val = is_shadow ? calculate_shadow(fs_in.frag_pos_light_space,
            bias_val) : 0;
    shadow_val = min(shadow_val, 0.75);
