...
void game_loop(GLFWwindow *win, const std::vector<GLuint> &VAO,
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        // drawing floor
        glStencilMask(0x00);
        draw_object(shad, VAO[1], tex_maps[1], view, proj, glm::mat4{1},
                num_verts[1]);
        // drawing cubes: 1st pass
        glStencilFunc(GL_ALWAYS, 1, 0xFF);
        glStencilMask(0xFF);
        draw_object(shad, VAO[0], tex_maps[0], view, proj,
                glm::translate(glm::mat4{1}, glm::vec3{-1, 0, -1}),
                num_verts[0]);
        draw_object(shad, VAO[0], tex_maps[0], view, proj,
                glm::translate(glm::mat4{1}, glm::vec3{2, 0, 0}), num_verts[0]);
        // drawing cubes: 2nd pass
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        glStencilMask(0x00);
        glDisable(GL_DEPTH_TEST);
        draw_object(color_shader, VAO[0], tex_maps[0], view, proj,
                glm::scale(glm::translate(glm::mat4{1}, glm::vec3{-1, 0, -1}),
                    glm::vec3{1.1}), num_verts[0]);
        draw_object(color_shader, VAO[0], tex_maps[0], view, proj,
                glm::scale(glm::translate(glm::mat4{1}, glm::vec3{2, 0, 0}),
                    glm::vec3{1.1}), num_verts[0]);
        glStencilMask(0xFF);
        glEnable(GL_DEPTH_TEST);
        glfwSwapBuffers(win);
