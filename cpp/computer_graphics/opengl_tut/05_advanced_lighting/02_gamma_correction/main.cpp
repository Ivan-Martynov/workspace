 ...
 *
 * Demonstrating gamma correction
 */
...
void test_gamma_correction(GLFWwindow*);
...
GLFWwindow* init(const GLuint w, const GLuint h) {
    ...
    GLFWwindow *win = glfwCreateWindow(w, h, "Gamma Correction",
            nullptr, nullptr);
    if (win == nullptr)
...
void test_gamma_correction(GLFWwindow* win) {
    GLuint vao {}, vbo {};
    gen_object_vao_vbo(vao, vbo, floor_vertices());

    // texture without and with gamma correction
    const std::string tex_fn {tex_path + "hardwood_floor.jpg"};
    const GLuint tex_id {load_texture(tex_fn, false)};
    const GLuint tex_id_gamma {load_texture(tex_fn, true)};

    // shader
    const Shader shad {shad_path + "blinn_phong_01.vs",
        shad_path + "gamma_corr_01.frag"};

    // light source positions and colors
    const std::vector<glm::vec3> lamps_pos = {{-3, 0, 0}, {-1, 0, 0},
        {1, 0, 0}, {3, 0, 0}};
    const std::vector<glm::vec3> lamps_clr = {glm::vec3{0.25}, glm::vec3{0.50},
        glm::vec3{0.75}, glm::vec3{1}};
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        // check if button 'G' has been pressed
        if (keys[GLFW_KEY_G] && !keys_pressed[GLFW_KEY_G]) {
            is_gamma = !is_gamma;
            keys_pressed[GLFW_KEY_G] = true;
        }
        ...
        glUniform3fv(glGetUniformLocation(idx, "light_pos"), 4,
                &lamps_pos[0][0]);
        glUniform3fv(glGetUniformLocation(idx, "light_color"), 4,
                &lamps_clr[0][0]);
        glUniform3fv(glGetUniformLocation(idx, "view_pos"), 1,
                &(main_cam.pos()[0]));
        glUniform1i(glGetUniformLocation(idx, "is_gamma"), is_gamma);

        glBindVertexArray(vao);
        glBindTexture(GL_TEXTURE_2D, is_gamma ? tex_id_gamma : tex_id);
