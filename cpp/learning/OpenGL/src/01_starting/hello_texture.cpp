...
{
    Marvin::WindowController window_controller {900, 600, "Hello, texture."};
    if (!window_controller.loaded())
    {
        return 1;
    }
    ...
    // We know the location is in ./bin/ which is given via parent_path.
    // Therefore, one more call to parent_path is needed in order to get to the
    // root directory.
    ...
    assets_path /= "assets";
    assets_path = std::filesystem::path {TUTORIAL_ROOT_PATH} / "assets";
    graphics_path = assets_path / "graphics";

    window_controller.set_resize_callback(framebuffer_size_callback);
    window_controller.set_key_callback(key_callback);

    glPointSize(5);
    while(!glfwWindowShouldClose(window_controller.get_pointer()))
    {
        render_window_start();
        ...
        render_window_end(window_controller.get_pointer());
    }
