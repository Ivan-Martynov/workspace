...
// use the recursive technique
template <size_t N, class Num>
inline Num det_recursive(const Mat<N, N, Num> &M) {
    Num res {};
    for (size_t i {0}; i < N; i += 2)
        res += M[i][0] * det_recursive(mat_minor(M, i, 0));
...
template <class Num>
constexpr Num det_recursive(const Mat<2, 2, Num> &M) {
    return M[0][0] * M[1][1] - M[0][1] * M[1][0];
}

template <class Num>
constexpr Num det_recursive(const Mat<1, 1, Num> &M) {
    return M[0][0];
}
...
inline Mat<N, N, double> invert_transpose(const Mat<N, N, Num> &M) {
    ...
    for (size_t i {0}; i < N; ++i)
        for (size_t j {0}; j < N; ++j)
            ret[i][j] = det(mat_minor(M, i, j)) * ((i + j) & 1 ? -1 : 1);
