...
public:
    ...
    using reverse_iterator = typename
        std::array<value_type, N>::reverse_iterator;
    using const_iterator   = typename
        std::array<value_type, N>::const_iterator;
    ...
    constexpr reverse_iterator rbegin() noexcept { return V_.rbegin(); }
    ...
    constexpr reverse_iterator rend() noexcept { return V_.rend(); }
    ...
    // index access
    constexpr value_type& operator[](const size_t i) noexcept { return V_[i]; }
    constexpr value_type operator[](const size_t i) const noexcept {
        return V_[i];
    }
    constexpr value_type& at(const size_t i) { return V_.at(i); }
    constexpr value_type at(const size_t i) const { return V_.at(i); }
    /*
    ...
    constexpr Vec& operator+=(const value_type rhs) noexcept {
        for (auto& x: V_) x += rhs; return *this;
    }
    constexpr Vec& operator+=(const Vec& rhs) noexcept {
        for (size_type i {0}; i < N; ++i) V_[i] += rhs[i]; return *this;
    }
    constexpr Vec& operator-=(const value_type rhs) noexcept {
        for (auto& x: V_) x -= rhs; return *this;
    }
    ...
    constexpr std::array<Num, N> array() const noexcept { return V_; }
...
// default constructor
template <size_t N, class Num>
constexpr Vec<N, Num>::Vec() noexcept: V_{} { }
...
template <size_t N, class Num>
Vec<N, Num>::Vec(const Num val) noexcept: V_{} { V_.fill(val); }
...
template <size_t N, class Num>
constexpr void Vec<N, Num>::normalize() noexcept { operator/=(norm()); }
