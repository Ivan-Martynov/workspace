...
template <class T>
std::ostream& operator<<(std::ostream&, const Generating_function<T>&);
...
template <class T> class Generating_function
{
  public:
    ...
    {
        seq_ = std::vector<T> {o.cbegin(), o.cend()};
        return *this;
    }
    ...
    //            std::is_same<T, std::common_type_t<T, U>>::value>>
    // no restrictions: may truncate
    template <class U> Generating_function& operator*=(const U rhs)
    {
    ...
    // index operator
    constexpr value_type operator[](const size_t i) const
    {
        return seq_[i];
    }
    ...
    const std::vector<T> sequence() const noexcept
    {
        return seq_;
    }
    ...
    template <class U> constexpr auto value(const U x) const
    {
        return poly_val_v0(seq_, x);
    }
  ...
  private:
    ...
    friend std::ostream& operator<< <>(
        std::ostream&, const Generating_function&);
};
...
auto operator+(
    const Generating_function<T>& lhs, const Generating_function<U>& rhs)
{
...
template <class T, class U>
auto operator+(Generating_function<T>&& lhs, const Generating_function<U>& rhs)
{
    // std::cout << "rvalue + version\n";
    return (std::is_same<T, std::common_type_t<T, U>>::value)
               ? std::move(lhs += rhs)
               : std::move(Generating_function<std::common_type_t<T, U>> {lhs}
                           += rhs);
}
// sum version with rhs as an rvalue
template <class T, class U>
auto operator+(const Generating_function<T>& lhs, Generating_function<U>&& rhs)
{
...
Generating_function<T> operator*(const Generating_function<T>& lhs, const U rhs)
{
    // std::cout << "simple * version\n";
    return Generating_function<std::common_type_t<T, U>> {lhs} *= rhs;
}
...
template <class T, class U,
    typename = std::enable_if_t<std::is_arithmetic<U>::value>>
Generating_function<T> operator*(const U lhs, const Generating_function<T>& rhs)
{
    return operator*(rhs, lhs);
}
