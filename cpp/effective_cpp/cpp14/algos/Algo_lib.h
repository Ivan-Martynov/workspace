...
constexpr static double sqrt_of_5 {sqrt(5)};
// golden ratio
constexpr static double phi {0.5 * (sqrt_of_5 + 1.0)};
constexpr static double golden_ratio() noexcept
{
    return phi;
}
...
template <size_t N> constexpr size_t fac_v2() noexcept
{
    return N * fac_v2<N - 1>();
}
template <> constexpr size_t fac_v2<0>() noexcept
{
    return 1;
}
...
// using templates
template <size_t N, size_t K> struct Binom_v3_s
{
    constexpr static size_t val
        = (K > N) ? 0
                  : Binom_v3_s<N - 1, K>::val + Binom_v3_s<N - 1, K - 1>::val;
};
template <size_t K> struct Binom_v3_s<0, K>
{
    constexpr static size_t val = 1;
};
template <size_t N> struct Binom_v3_s<N, 0>
{
    constexpr static size_t val = 1;
};
template <size_t N> struct Binom_v3_s<N, N>
{
    constexpr static size_t val = 1;
};
template <> struct Binom_v3_s<0, 0>
{
    constexpr static size_t val = 1;
};

template <size_t N, size_t K> constexpr size_t binom_v3()
{
...
// using templates
template <size_t N> constexpr double harmonic_v2() noexcept
{
    return 1.0 / N + harmonic_v2<N - 1>();
}
template <> constexpr double harmonic_v2<0>() noexcept
{
    return 0;
}
...
// using templates: can accept only const parameter
template <size_t N> constexpr size_t fib_v3() noexcept
{
    return fib_v3<N - 1>() + fib_v3<N - 2>();
}
template <> constexpr size_t fib_v3<1>() noexcept
{
    return 1;
}
template <> constexpr size_t fib_v3<0>() noexcept
{
    return 0;
}
...
constexpr int stirling_kind1_v0(const size_t n, const size_t k)
{
    return (k == 0)   ? ((n == 0) ? 1 : 0)
           : (n == 0) ? 0
                      : stirling_kind1_v0(n - 1, k - 1)
                            - (n - 1) * stirling_kind1_v0(n - 1, k);
}
...
constexpr size_t stirling_kind1_v1(const size_t n, const size_t k)
{
    return (k == 0)   ? ((n == 0) ? 1 : 0)
           : (n == 0) ? 0
                      : stirling_kind1_v1(n - 1, k - 1)
                            + (n - 1) * stirling_kind1_v1(n - 1, k);
}
...
constexpr size_t stirling_kind2_v0(const size_t n, const size_t k)
{
    return (k == 0)   ? 0
           : (n == k) ? 1
                      : stirling_kind2_v0(n - 1, k - 1)
                            + k * stirling_kind2_v0(n - 1, k);
}
