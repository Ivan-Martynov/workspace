 ...
 * changed values
 */
bool clip_line(int& x1, int& y1, int& x2, int& y2, const int xmin,
...
void draw_horizontal_line(
    RGB_Image& I, const RGB_Color& c, const int x1, const int x2, const int y)
{
    const uint clr {c.color()};
...
void clip_n_draw_horizontal_line(
    RGB_Image& I, const RGB_Color& c, const int x1, const int x2, const int y)
{
    const int w = I.width() - 1;
    draw_horizontal_line(
        I, c, std::min(std::max(0, x1), w), std::min(std::max(0, x2), w), y);
}

void draw_vertical_line(
    RGB_Image& I, const RGB_Color& c, const int x, const int y1, const int y2)
{
    const uint clr {c.color()};
...
void clip_n_draw_vertical_line(
    RGB_Image& I, const RGB_Color& c, const int x, const int y1, const int y2)
{
    const int h = I.height() - 1;
    draw_vertical_line(
        I, c, x, std::min(std::max(0, y1), h), std::min(std::max(0, y2), h));
}
...
void Triangle::fill_hs(RGB_Image& I, const RGB_Color& C) const
{
    ...
    xmin &= ~(q - 1);
    ymin &= ~(q - 1); // start in corner of qxq block
 ...
 */
Vec<3, double> bary(
    const Point& p1, const Point& p2, const Point& p3, const Point& p)
{
         ...
         y < std::min(std::max(0, ymax), h); ++y)
    {
        ...
        for (size_t i {0}, j {n - 1}; i < n; j = i++)
        {
            ...
            if ((yi < y && yj >= y) || (yj < y && yi >= y))
            {
                ...
                nodes.push_back(
                    xi + (double(y) - yi) / (yj - yi) * (pa_[j].x() - xi));
            }
        }
...
void Circle::doDraw(RGB_Image& I, const RGB_Color& c) const
{
    ...
    while (x <= y)
    {
        ...
        I.set_color(xc + x, yc + y, c);
        I.set_color(xc - x, yc + y, c);
        I.set_color(xc + x, yc - y, c);
        I.set_color(xc - x, yc - y, c);
        I.set_color(xc + y, yc + x, c);
        I.set_color(xc - y, yc + x, c);
        I.set_color(xc + y, yc - x, c);
        I.set_color(xc - y, yc - x, c);
    }
    I.set_color(xc, yc + r_, c);
    I.set_color(xc, yc - r_, c);
    I.set_color(xc + r_, yc, c);
    I.set_color(xc - r_, yc, c);
}
